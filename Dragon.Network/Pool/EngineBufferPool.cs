...
public sealed class EngineBufferPool : IEngineBufferPool {
    ...
    public EngineBufferPool(int readerLength, int writerLength, int bufferReaderSize, int bufferWriterSize) {
        ...
        for (var i = 0; i < readerLength; ++i) {
            readers.Add(new EngineBufferReader(bufferReaderSize));
        }

        for (var i = 0; i < writerLength; ++i) {
            writers.Add(new EngineBufferWriter(bufferWriterSize));
        }
    }
